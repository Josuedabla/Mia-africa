...
export default function CheckoutPage() {
  ...
  const detectPosition = () => {
    navigator.geolocation.getCurrentPosition(
      ...
      () => setError('Impossible de détecter votre position. Vous pouvez continuer avec juste votre adresse.')
    );
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setError(err?.message ?? 'La commande a échoué. Vérifiez votre panier et réessayez.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4 max-w-lg mx-auto">
      <h1 className="text-xl font-bold text-gray-900 mb-1">Finaliser la commande</h1>
      {shopCount > 1 && (
        <p className="text-sm text-gray-500 mb-6">
          Votre panier contient des produits de {shopCount} boutiques différentes : {shopCount} commandes séparées
          seront créées, chaque vendeur ne recevra que ce qui le concerne.
        </p>
      )}
      ...
      <div className="bg-white rounded-xl shadow-sm p-4 space-y-4 mb-4">
        ...
        >
          <MapPin size={16} />
          {coords ? 'Position détectée ✓' : 'Utiliser ma position actuelle'}
        </button>
      </div>

      <div className="bg-white rounded-xl shadow-sm p-4 space-y-4 mb-4">
        <div>
          <span className="text-sm font-medium text-gray-700 block mb-2">Paiement du produit</span>
        ...
        <div>
          <span className="text-sm font-medium text-gray-700 block mb-2">Paiement de la livraison</span>
          <div className="flex gap-2">
            <button
      ...
      >
        ...
        Confirmer la commande ({cart.totalAmount.toLocaleString()} XOF)
      </button>
    </div>
  );
}
