...
import { Search, MapPin, Bell, Wallet as WalletIcon, Heart, Flame, Sparkles, Trophy, Navigation, Loader2, User } from 'lucide-react';
...
export default function HomePage() {
  ...
  const { countryCode, requestPreciseLocation } = useCountry();
  ...
  useEffect(() => {
    ...
    setLoading(true);
    Promise.all([
      getTrendingProducts(countryCode, 10),
      getNewProducts(countryCode, 10),
      getBestSellers(countryCode, 10),
    ])
  ...
  useEffect(() => {
    ...
    const handle = setTimeout(async () => {
      try {
        const results = await searchProducts(searchQuery, { country: countryCode, limit: 24 });
  ...
  const handleFindNearby = useCallback(async () => {
    ...
    try {
      ...
      navigator.geolocation.getCurrentPosition(async (position) => {
        const shops = await nearbyShops(position.coords.latitude, position.coords.longitude, 20, countryCode);
        setNearby(shops);
  ...
  }, [requestPreciseLocation, countryCode]);
        ...
        <div className="max-w-6xl mx-auto px-4 py-3 flex items-center gap-3">
          ...
          <div className="flex-1 relative">
            ...
            <input
              ...
              placeholder="Chaussures, robes, téléphones..."
          ...
          <span className="hidden sm:inline-flex items-center gap-1 text-xs font-semibold text-gray-500 bg-gray-100 rounded-full px-3 py-1.5 shrink-0">
            <MapPin size={13} /> {COUNTRY_LABELS[countryCode] ?? countryCode}
          </span>
              ...
              <section>
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-2">
                    ...
                    <h2 className="font-bold text-gray-900 text-lg">Boutiques près de vous</h2>
                  </div>
                  {nearby === null && (
                    <button
                ...
                {nearby === null && (
                  <p className="text-sm text-gray-400">Activez votre position pour découvrir les boutiques proches de vous.</p>
                )}
                {nearby !== null && nearby.length === 0 && (
                  <p className="text-sm text-gray-400">Aucune boutique trouvée près de vous pour le moment.</p>
                )}
                {nearby !== null && nearby.length > 0 && (
            ...
            <section>
              <div className="flex items-center gap-2 mb-3">
                ...
                <h2 className="font-bold text-gray-900 text-lg">
                  {searchResults ? `Résultats pour "${searchQuery}"` : 'Pour vous'}
                </h2>
              </div>
              ...
              {searchResults ? (
                <>
                  {visibleSearchResults?.length === 0 && (
                    <p className="text-sm text-gray-400 py-8 text-center">Aucun résultat pour cette recherche.</p>
                  )}
                ...
                <MasonryFeed country={countryCode} category={activeCategory} />
              )}
            </section>
          </>
        )}
