 ...
 *
 * Admin status is no longer an email compared against an env var (a
 * pattern that was actually broken end-to-end in the Firebase version -
 * process.env never resolves under Vite). It's now the `is_admin`
 * boolean on the profiles row, which only a service-role script or a
 * trusted Postgres migration can set - never the client itself (RLS's
 * profiles_update_own policy explicitly blocks flipping is_admin).
 */
