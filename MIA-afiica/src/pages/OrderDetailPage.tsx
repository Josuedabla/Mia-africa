...
function ReviewForm({ orderId, productId, productName }: { orderId: string; productId: string; productName: string }) {
  ...
  if (existingReview || submitted) {
    return (
      <div className="bg-gray-50 rounded-lg p-3 mt-2">
        <p className="text-sm font-medium text-gray-700 mb-1">Votre avis sur {productName}</p>
  ...
  return (
    <div className="bg-gray-50 rounded-lg p-3 mt-2">
      <p className="text-sm font-medium text-gray-700 mb-2">Laisser un avis sur {productName}</p>
...
export default function OrderDetailPage() {
  ...
  if (!order) return <div className="text-center py-16 text-gray-500">Commande introuvable.</div>;
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4 max-w-lg mx-auto">
      ...
      <p className="text-sm text-gray-500 mb-6">Commande du {new Date(order.created_at).toLocaleDateString('fr-FR')}</p>
      ...
      {delivery && delivery.status !== 'delivered' && delivery.otp_code && (
        <div className="bg-mia-green-50 border border-mia-green-200 rounded-xl p-4 mb-4 text-center">
          <div className="flex items-center justify-center gap-2 text-mia-green-700 mb-2">
            <KeyRound size={18} />
            <span className="text-sm font-medium">Code de remise à donner au livreur</span>
          </div>
