...
export default function DriverDashboard() {
  ...
  useEffect(() => {
    navigator.geolocation.getCurrentPosition(
      ...
      () => setError("Impossible d'obtenir votre position. Activez la géolocalisation pour voir les livraisons proches de vous.")
    );
  }, []);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setError('Impossible de charger les livraisons disponibles.');
    } finally {
      setLoading(false);
    }
  }, [position]);
  ...
  const handleClaim = async () => {
    ...
    try {
      ...
      setError('Certaines livraisons ont déjà été prises par un autre livreur. Liste actualisée.');
      setSelected(new Set());
      await load();
    } finally {
      setClaiming(false);
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-24">
      <div className="bg-white shadow-sm sticky top-0 z-20 px-4 py-4">
        <h1 className="text-xl font-bold text-gray-900">Livraisons disponibles</h1>
        <p className="text-sm text-gray-500">Choisissez une ou plusieurs livraisons à récupérer</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-16 text-gray-500">Aucune livraison disponible pour le moment près de vous.</div>
      ) : (
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-3 text-xs text-gray-500 mt-1">
                        <span className="flex items-center gap-1">
                          <Package size={12} /> {d.item_count} article(s)
                        </span>
                        <span className="flex items-center gap-1">
      ...
      {selected.size > 0 && (
        <div className="fixed bottom-0 inset-x-0 bg-white border-t border-gray-200 p-4 shadow-lg">
          ...
          >
            ...
            Récupérer {selected.size} livraison{selected.size > 1 ? 's' : ''}
          </button>
        </div>
      )}
    </div>
