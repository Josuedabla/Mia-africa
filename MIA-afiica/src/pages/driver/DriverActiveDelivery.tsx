...
export default function DriverActiveDelivery() {
  ...
  const load = async () => {
    ...
    const { data, error: fetchError } = await supabase
      ...
      .eq('id', deliveryId)
      .single();
    if (fetchError) setError('Impossible de charger cette livraison.');
  ...
  const handlePickedUp = async () => {
    ...
    try {
      ...
      setError("Impossible de marquer le colis comme récupéré.");
    } finally {
  ...
  const handleConfirmDelivery = async () => {
    ...
    try {
      ...
      setError(err?.message?.includes('INVALID_OTP') ? 'Code incorrect. Demandez au client de vous redonner le code.' : 'La confirmation a échoué.');
    } finally {
      setActionLoading(false);
    }
  };
  ...
  const handleFailed = async () => {
    if (!deliveryId) return;
    if (!confirm('Confirmer que cette livraison a échoué (client injoignable, adresse introuvable...) ?')) return;
    setActionLoading(true);
    try {
  ...
  if (!delivery) {
    return <div className="text-center py-16 text-gray-500">Livraison introuvable.</div>;
  }
    ...
    <div className="min-h-screen bg-gray-50 p-4 max-w-lg mx-auto">
      ...
      {delivery.status === 'assigned' && (
        <div className="bg-white rounded-xl shadow-sm p-5 text-center">
          ...
          <p className="text-gray-700 mb-4">Récupérez la commande chez le vendeur</p>
          <button
          ...
          >
            {actionLoading ? <Loader2 className="animate-spin mx-auto" size={20} /> : 'Colis récupéré'}
          </button>
        </div>
      )}
      ...
      {delivery.status === 'picked_up' && (
        <div className="bg-white rounded-xl shadow-sm p-5">
          <p className="text-gray-700 mb-3 text-center">
            Demandez au client le code à 4 chiffres pour confirmer la remise
          </p>
          <input
            value={otpInput}
          ...
          <button onClick={handleFailed} className="w-full text-sm text-red-600 py-2">
            Signaler un problème (client injoignable...)
          </button>
        </div>
      )}
      ...
      {delivery.status === 'delivered' && (
        <div className="bg-white rounded-xl shadow-sm p-5 text-center">
          ...
          <p className="text-gray-700 font-semibold">Livraison confirmée</p>
        </div>
      )}
    </div>
  );
