  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <KpiCard icon={Package} label="Produits en ligne" value={String(shop.product_count ?? 0)} accent="bg-blue-50 text-blue-600" />
        <KpiCard icon={ShoppingBag} label="Ventes totales" value={String(shop.total_sales ?? 0)} accent="bg-purple-50 text-purple-600" />
        <KpiCard icon={Star} label="Note boutique" value={shop.rating ? shop.rating.toFixed(1) : '—'} accent="bg-amber-50 text-amber-600" />
        <KpiCard icon={TrendingUp} label="Score vendeur" value={`${shop.seller_score}/100`} accent="bg-mia-green-50 text-mia-green-600" />
      </div>
