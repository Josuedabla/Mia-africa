...
import { buildProductShareUrl, buildProductShareChannels, shareNative, copyToClipboard, type ShareChannel } from '@/services/share.service';
...
export default function ShareSheet({ productId, productName, price, onClose }: ShareSheetProps) {
  const [copied, setCopied] = useState(false);
  const productUrl = buildProductShareUrl(productId);
  const channels = buildProductShareChannels({ productId, productName, price });
  ...
  const handleChannelClick = async (channel: ShareChannel) => {
    ...
    if (channel.id === 'native') {
      const didShare = await shareNative({ title: productName, text: productName, url: productUrl });
