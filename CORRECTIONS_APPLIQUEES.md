...
## Boost produit — expiration jamais appliquée (bug financier) + tri tout-ou-rien corrigé

- **Bug 1 (le plus grave)** : `boost_product()` (migration `20260718000006_functions.sql`) débite le vendeur en pièces, enregistre une ligne `product_boosts` avec une date d'expiration (`expires_at`), puis met `products.is_boosted = true` — mais **rien nulle part** ne repassait `is_boosted` à `false` une fois `expires_at` dépassé. Un vendeur payant pour 24h de boost restait donc boosté indéfiniment, gratuitement, après ce délai.
- **Bug 2** : tous les tris utilisant `is_boosted` (`search_products`, `get_discovery_feed_page`, `get_trending_products_capped`) le traitaient comme un simple booléen — payer 100 pièces ou 10 000 pièces de boost produisait exactement le même effet. La colonne `boost_score` (proportionnelle au montant payé) existait déjà en base mais n'était utilisée nulle part.
- **Migration `20260723000029_boost_expiration_and_scoring.sql`** :
  - `get_active_boost_score(product_id)` : somme des `boost_score` des boosts **encore actifs** (`expires_at > now()`) pour un produit — 0 si aucun boost actif.
  - `expire_product_boosts()` : repasse `is_boosted` à `false` pour tout produit sans boost actif restant (et, par sécurité symétrique, à `true` si un score actif existe mais que la colonne était désynchronisée). Idempotente, prévue pour un cron.
  - `search_products`, `get_trending_products_capped`, `get_discovery_feed_page` : `order by is_boosted desc` remplacé par `order by get_active_boost_score(...) desc` — payer plus a maintenant un effet réel et proportionnel sur le classement.
  - **Rattrapage immédiat** : la migration appelle `expire_product_boosts()` une fois à son exécution, pour corriger tout de suite les produits déjà désynchronisés par le bug, sans attendre le premier passage du cron.
- **Anti-monopole préservé** : `get_trending_products_capped` applique toujours son plafond `max_per_shop` **avant** le tri par score de boost (migration 018) — renforcer l'effet du boost ne permet donc jamais à un vendeur de dépasser ce plafond, peu importe combien il paie.
- **Nouvelle Edge Function `supabase/functions/expire-boosts-cron/index.ts`** : appelle `expire_product_boosts()`, protégée par le même secret `CRON_SECRET` que `leaderboard-cron`. À planifier toutes les 15 minutes (Dashboard Supabase > Edge Functions > Schedule) — voir section ajoutée dans `scripts/set-edge-function-secrets.sh` pour la génération du secret et le planning recommandé des deux crons.
- **Frontend** : aucun changement nécessaire — `HomePage.tsx` affiche déjà le badge "🚀 Boosté" à partir de `product.is_boosted`, qui reflétera maintenant correctement l'état réel une fois le cron actif ; aucun composant ne lit `boost_score` directement (il ne sert qu'au tri côté SQL).
- **Point de vigilance signalé, pas résolu ici** : `get_active_boost_score()` est appelée par ligne dans les `order by` (sous-requête non indexée) — acceptable pour un catalogue de taille modeste, mais deviendra un point de lenteur si le nombre de produits actifs grandit beaucoup. À revoir avec une colonne dénormalisée mise à jour par trigger si ça devient un problème réel en pratique.

---


## Point 3 (suite — domaine personnalisé) — Automatisation Vercel, plus d'étape manuelle admin

- **Contexte** : la migration `20260723000028_shop_product_slugs_and_domain.sql` stockait déjà la demande de domaine (`custom_domain`, statut `pending`/`verified`/`failed`) mais son propre commentaire indiquait explicitement que l'attachement réel au projet Vercel restait une "étape manuelle admin hors base de données" — non viable pour une seule personne gérant potentiellement de nombreux vendeurs.
- **Nouvelle Edge Function `supabase/functions/vercel-domain-manager/index.ts`** : appelle directement l'API REST Vercel (`POST /v10/projects/{id}/domains`, `GET /v9/projects/{id}/domains/{domain}`, `GET /v6/domains/{domain}/config`) avec un token stocké en secret serveur (`VERCEL_API_TOKEN`, `VERCEL_PROJECT_ID`, jamais exposés au client). Deux actions :
  - `add` : attache le domaine au projet Vercel juste après que la RPC SQL l'a validé et réservé, renvoie les instructions DNS exactes (A pour un domaine apex, CNAME sinon) à afficher au vendeur.
  - `check` : réinterroge Vercel pour savoir si les DNS pointent correctement et le SSL est bon (`verified && !misconfigured`), et ne fait passer `custom_domain_status` à `verified` que dans ce cas — le client ne peut jamais forcer ce statut lui-même, exactement comme la RPC SQL l'imposait déjà.
- **`src/services/db.service.ts`** : `requestShopCustomDomain()` enchaîne maintenant la RPC SQL puis l'appel à `vercel-domain-manager` (action `add`). Nouvelle fonction `checkShopCustomDomainStatus()` (action `check`).
- **`VendorSettings.tsx`** : affiche l'enregistrement DNS exact à créer chez le registrar (type/nom/valeur), et un bouton "Vérifier maintenant" qui déclenche `check` à la demande.
- **Secrets à définir avant déploiement** (`scripts/set-edge-function-secrets.sh`, section ajoutée) : `VERCEL_API_TOKEN`, `VERCEL_PROJECT_ID`, et `SUPABASE_ANON_KEY` (celle-ci n'est **pas** injectée automatiquement dans les Edge Functions contrairement à `SUPABASE_URL`/`SUPABASE_SERVICE_ROLE_KEY` — nécessaire ici pour vérifier via RLS que l'appelant est bien propriétaire de la boutique avant de toucher à Vercel).
- **Hors périmètre de cette passe** :
  - Pas de cron automatique qui repasse `pending → verified` tout seul (le vendeur doit cliquer "Vérifier maintenant"). Un cron réutilisant le pattern de `leaderboard-cron` pourrait appeler `check` périodiquement pour toutes les boutiques `pending` — pas fait ici, à ajouter si le clic manuel s'avère insuffisant en usage réel.
  - `VERCEL_TEAM_ID` prévu dans le code et le script de secrets mais non nécessaire aujourd'hui (compte personnel, pas de Team Vercel) — à définir seulement si le projet migre un jour vers une Team.

---

## Point 3 (suite — domaine personnalisé) — Détachement Vercel au retrait (action `remove`)

- **Correctif du point précédent** : `removeShopCustomDomain()` ne faisait que remettre `shops.custom_domain` à `null` côté SQL, sans jamais détacher le domaine du projet Vercel — un domaine retiré restait donc orphelin sur Vercel indéfiniment.
- **Edge Function `vercel-domain-manager`** : nouvelle action `remove` (`DELETE /v9/projects/{id}/domains/{domain}` côté API Vercel). Un `404` de Vercel (domaine déjà absent) est traité comme un succès, pas une erreur.
- **Point d'attention sur l'ordre des appels** : contrairement à `add` (appelée après la RPC SQL, qui a déjà validé et réservé le domaine), `remove` doit être appelée **avant** la RPC `remove_shop_custom_domain()` — parce que cette RPC met `shops.custom_domain` à `null`, et sans le domaine encore présent en base, l'Edge Function ne saurait ni quel domaine détacher de Vercel, ni vérifier que l'appelant en est bien propriétaire. `removeShopCustomDomain(shopId, currentDomain)` a donc une nouvelle signature (le domaine actuel doit être transmis par l'appelant) et respecte cet ordre : Vercel d'abord, RPC SQL ensuite.
- **Sécurité** : l'Edge Function n'accepte de détacher que le domaine qui correspond exactement à `shops.custom_domain` relu via RLS au moment de l'appel — un `domain` arbitraire dans le body est rejeté (`DOMAIN_MISMATCH`), ce qui empêche un appelant de faire détacher le domaine Vercel d'une autre boutique en le devinant.
- **Résilience volontaire** : si l'appel Vercel échoue (réseau, token expiré...), le retrait côté MIA n'est **pas** bloqué — le vendeur revient au sous-domaine gratuit dans tous les cas, l'échec est seulement loggé. Un domaine orphelin sur Vercel sans DNS pointant dessus n'a aucun effet visible ; mieux vaut ça qu'un vendeur bloqué sur le bouton "Retirer".
- **Hors périmètre** : pas de tâche de nettoyage périodique des domaines orphelins côté Vercel (cas rare : seulement si l'appel `remove` échoue). À envisager seulement si ça devient un problème réel en pratique.

---


## Point 3 (MIA-Spec-Fonctionnalites.md, section 3.b) — Téléphone + sélecteur de pays

- **Nouveau composant `src/components/PhoneCountryInput.tsx`**, basé sur `react-phone-number-input` (lib recommandée par la spec). Drapeau + indicatif liés au même contrôle, `countryCallingCodeEditable=false` pour éviter un indicatif incohérent avec le drapeau, valeur toujours normalisée en E.164 (`+22890000000`) — compatible avec `profiles.phone` (unique) et `supabase.auth.signInWithOtp({ phone })`. Pays par défaut branché sur `useCountry()` (auto-détection déjà en place), jamais un pays figé.
- **`VendorOnboarding.tsx`** : l'ancien `<input type="text" placeholder="+228 90 00 00 00">` (texte libre, aucune validation) est remplacé par `PhoneCountryInput`. Validation ajoutée avant soumission (`isValidPhoneNumber`) — bloque l'appel à `become_seller` si le numéro est invalide au lieu de laisser Postgres/Twilio échouer plus tard silencieusement.
- **Style** : règles ajoutées dans `src/index.css` (`.mia-phone-input`) pour que le composant hérite du look MIA (bordures, focus ring vert) au lieu du style par défaut de la librairie.
- **Dépendance ajoutée** : `react-phone-number-input` dans `package.json` (`npm install` requis avant build/déploiement).
- **Recherché mais non trouvé ailleurs dans le code** : aucun autre champ téléphone en texte libre (checkout, profil livreur, compte) — `AccountPage.tsx` n'affiche le téléphone qu'en lecture seule, et il n'y a pas encore de formulaire d'édition de profil acheteur. Si un tel formulaire est ajouté plus tard, réutiliser `PhoneCountryInput`.
- **Hors périmètre de cette passe (volontairement)** : la connexion par téléphone/OTP (`signInWithPhone`/`verifyPhoneOtp` dans `useAuth.ts`) existe déjà côté hook mais n'a toujours aucune UI dans `AuthPage.tsx` (qui ne propose que email/mot de passe) — pas touché ici pour rester sur "remplacer le champ existant" comme demandé ; peut être branché avec le même composant dans une prochaine étape si souhaité.

---

---

## Point 3 (suite — MIA-Spec-Fonctionnalites.md, section 3.c) — i18n (fr/en)

- **Infra i18next** : `src/i18n/index.ts` (config + résolution initiale localStorage > navigateur > `fr`), `src/i18n/locales/fr.json` et `en.json` (clés `common.*`, `nav.*`, `auth.*`, `account.*`, `vendor_onboarding.*`). Branché au tout début de `main.tsx`.
- **`useLanguage.ts`** (nouveau hook) : résolution automatique `profiles.language` (si connecté) > pays détecté par `useCountry()` (Ghana/Nigeria/Kenya → anglais, reste des pays actuels → français) > langue du navigateur > `fr`. Contrairement à `useCountry()`, la langue reste modifiable explicitement à tout moment par l'utilisateur (bouton FR/EN) sans jamais être écrasée ensuite par la détection automatique une fois un choix fait.
- **`LanguageSwitcher.tsx`** (nouveau composant) : toggle compact FR/EN, ajouté sur `AuthPage.tsx`, `AccountPage.tsx` et le header de `HomePage.tsx`.
- **Pages entièrement traduites cette passe** : `AuthPage.tsx`, `AccountPage.tsx`, `VendorOnboarding.tsx` (labels, boutons, erreurs), header + puce catégorie "Tout" de `HomePage.tsx`. `PhoneCountryInput.tsx` traduit aussi (message d'erreur + libellés pays de la librairie selon la langue active).
- **Dépendances ajoutées** : `i18next`, `react-i18next` (`npm install` requis).
- **Hors périmètre de cette passe (à faire dans une prochaine itération, même infra à réutiliser)** : le reste de `HomePage.tsx` (sections Tendances/Nouveautés/Boutiques, "Pour toi"...), `ProductPage.tsx`, `ShopPage.tsx`, `CartDrawer.tsx`, `CheckoutPage.tsx`/`CheckoutGroupPage.tsx`, tout `src/pages/vendor/*` sauf `VendorOnboarding.tsx`, `src/pages/driver/*`, `src/pages/admin/*`, pages légales (`CGU`/`CGV`/`Confidentialite`), `SupportCenter.tsx`, `ReferralPage.tsx`, `CoinsPage.tsx`, `LeaderboardPage.tsx`. Les noms de catégories produit (Mode, Électronique...) ne sont pas traduits — ce sont des valeurs stockées telles quelles en base, les traduire nécessiterait de passer à des codes de catégorie + mapping, non fait ici.
- **Non implémenté (mentionné dans la spec mais pas demandé explicitement ici)** : traduction automatique du contenu généré par les vendeurs (descriptions produit, noms de boutique) — l'i18n mis en place ne couvre que l'interface (UI chrome), pas le contenu utilisateur.

---

## Point 2 (MIA-Spec-Fonctionnalites.md, section 3.a partielle + priorisation) — Variantes produit, description riche, vidéo

- **Variantes produit (couleur/taille/poids)** — n'existait pas du tout avant cette passe. Ajouté :
  - Migration `supabase/migrations/20260723000026_product_variants.sql` : tables `product_variant_attributes` (quels attributs sont actifs pour un produit + leurs valeurs possibles) et `product_variants` (une ligne par combinaison réellement vendue, avec son propre stock), RLS identique au reste du schéma, colonne `products.has_variants`, et un trigger qui garde `products.stock` synchronisé avec la somme des stocks de variantes — le checkout existant (qui ne décrémente que `products.stock`) continue de fonctionner sans modification.
  - `VendorProductForm.tsx` : section "Variantes" — toggles Couleur/Taille/Poids, ajout de valeurs par attribut, grille de stock par combinaison générée automatiquement (produit cartésien), stock global recalculé et affiché en lecture seule dès qu'au moins une variante est active.
  - `ProductPage.tsx` : sélecteurs de variante côté acheteur, stock et bouton "Ajouter au panier" qui suivent la combinaison sélectionnée, libellé de variante transmis au panier.
- **Description riche (HTML)** — le champ et l'éditeur WYSIWYG existaient déjà côté formulaire vendeur, mais **bug découvert et corrigé** : `ProductPage.tsx` affichait `product.description` en texte brut (`{product.description}`), donc les balises HTML s'affichaient littéralement au lieu d'être rendues. Corrigé avec un rendu HTML sanitizé (`dangerouslySetInnerHTML` + `sanitizeProductHtml`, même fonction de sanitization que côté sauvegarde).
- **Vidéo produit** — le champ et le lecteur existaient déjà pour YouTube et TikTok (`MediaPlayer.tsx`, `products.external_video_url`). **Vimeo ajouté** (parsing du lien, embed, libellé, message de validation du formulaire vendeur) pour correspondre à la spec ("YouTube/TikTok/Vimeo").
...
## Favoris — bouton coeur non branché sur ProductPage (correctif ajouté après audit)

- **Bug** : le bouton "Ajouter aux favoris" (icône coeur) sur `ProductPage.tsx` n'avait **aucun `onClick`** - purement décoratif, aucune table ni logique derrière.
- **Migration `20260728000040_favorites.sql`** : table `favorites` (`user_id`, `product_id`, unique par paire), RLS stricte (chacun ne voit/modifie que ses propres favoris), fonction `toggle_favorite(product_id)` (bascule atomique ajout/retrait en une seule RPC, évite une race condition sur double-clic), et `get_my_favorite_products()` pour une future page "Mes favoris".
- **`src/services/db.service.ts`** : `toggleFavorite()`, `isProductFavorited()`, `getMyFavoriteProducts()`.
- **Nouveau hook `src/hooks/useFavorite.ts`** : état favori d'un produit, mise à jour optimiste (annulée si l'appel échoue), redirection implicite vers `/connexion` gérée par l'appelant si `UNAUTHENTICATED`.
- **`ProductPage.tsx`** : bouton câblé (`handleToggleFavorite`), état visuel dynamique (coeur rouge rempli si favori), redirige vers `/connexion` si l'utilisateur n'est pas connecté.
- **i18n** : nouvelle clé `product.remove_from_favorites` ajoutée dans les 5 langues (fr, en, pt, sw, ar).
- **Hors périmètre** : pas de page "Mes favoris" dédiée créée ici (la RPC `get_my_favorite_products` existe et est prête, mais aucune route/page ne l'appelle encore) - à ajouter si besoin.
