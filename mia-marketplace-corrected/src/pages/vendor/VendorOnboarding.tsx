/**
 * Vendor onboarding: the real "Créer ma boutique" flow that
 * DevenirVendeur.tsx's button used to have no handler for at all.
 * Calls the becomeVendor Cloud Function (functions/src/vendor.ts).
 */
...
export default function VendorOnboarding() {
  ...
  const [form, setForm] = useState({ shopName: '', category: CATEGORIES[0], country: COUNTRIES[0].code, phone: '' });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError(null);
    try {
      const becomeVendor = httpsCallable(functions, 'becomeVendor');
      await becomeVendor(form);
      navigate('/vendeur/dashboard', { replace: true });
    } catch (err: any) {
