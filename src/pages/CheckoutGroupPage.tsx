  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4 max-w-lg mx-auto">
      <div className="text-center mb-6">
        ...
        <h1 className="text-xl font-bold text-gray-900">{t('checkout_group.title')}</h1>
        <p className="text-sm text-gray-500">
          {orders.length > 1
            ? t('checkout_group.orders_created_plural', { count: orders.length })
            : t('checkout_group.order_created_single')}
        </p>
      </div>
        ...
        {orders.map((order) => (
          <div key={order.id} className="bg-white rounded-xl shadow-sm p-4">
            ...
            <div className="flex justify-between font-bold text-gray-900 pt-2 border-t border-gray-100">
              <span>{t('checkout_group.total')}</span>
              <span>{order.total?.toLocaleString()} XOF</span>
            </div>
            <p className="text-xs text-gray-400 mt-1">{t('checkout_group.status_label')} {order.status}</p>
          </div>
        ))}
      </div>
      ...
      >
        {t('checkout_group.continue_shopping')}
      </Link>
    </div>
  );
}
