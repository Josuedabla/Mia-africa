...
import {
  ...
  listCoinPurchaseTiers,
  claimDailyLoginReward,
  listRewardTasks,
  claimRewardTask,
  type RewardTask,
  type CoinPurchaseTier,
...
export default function CoinsPage() {
  ...
  const [tiers, setTiers] = useState<CoinPurchaseTier[]>([]);
  ...
  const handlePurchase = async () => {
    ...
    setMessage(null);
    try {
      const { checkoutUrl } = await initiateCoinPurchase(selectedTier.coin_amount, `${window.location.origin}/pieces`);
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-gray-300">
                      {new Date(tx.created_at).toLocaleString(i18n.language, {
                        day: '2-digit',
                        month: 'short',
                        year: 'numeric',
                        hour: '2-digit',
            ...
            {tiers.map((tier) => (
              <button
                ...
                className={`py-3 rounded-lg border text-sm font-semibold flex flex-col items-center ${
                  selectedTierId === tier.id ? 'border-mia-green-600 bg-mia-green-50 text-mia-green-700' : 'border-gray-200 text-gray-600'
                }`}
              >
                <span>{tier.coin_amount.toLocaleString()} 🪙</span>
                <span className="text-[11px] text-gray-400 font-normal">{tier.price_fcfa.toLocaleString()} FCFA</span>
              </button>
            ))}
          </div>
          <button
            onClick={handlePurchase}
            disabled={purchaseLoading || !selectedTier}
          ...
          >
            ...
            {selectedTier &&
              t('coins_page.buy_button', {
                amount: selectedTier.coin_amount.toLocaleString(),
                price: selectedTier.price_fcfa.toLocaleString(),
              })}
          </button>
