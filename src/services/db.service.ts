...
/**
 * "✨ Nouveautés" - AUCUN filtre pays (décision produit explicite :
 * MIA est panafricain, pas de barrière géographique sur les surfaces de
 * découverte publiques, sauf sur les produits similaires). Le paramètre
 * `country` n'est plus utilisé ici; conservé dans la signature pour ne
 * pas casser les appelants existants.
 */
export async function getNewProducts(_country: string, limit = 12): Promise<Product[]> {
...
export async function getProductsByCategory(category: string, _country: string, limit = 20): Promise<Product[]> {
  const { data } = await supabase
    .from('products')
    .select('*')
...
// ---------------------------------------------------------------------
// Favorites (produits sauvegardés)
// ---------------------------------------------------------------------
// toggle_favorite fait l'aller-retour insert/delete en une seule RPC
// atomique (voir migration 20260728000040_favorites.sql) plutôt qu'un
// select suivi d'un insert/delete côté client, pour éviter une race
// condition sur un double-clic rapide sur le bouton favoris.
...
export async function getMyFavoriteProducts(): Promise<Product[]> {
