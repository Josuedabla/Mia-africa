...
/** Lien direct /produit/:slugOrId, pour "Copier le lien" - product-meta reste le lien utilisé pour les réseaux sociaux (aperçu avec image filigranée), mais un lien direct plus lisible est préférable pour un copier-coller affiché tel quel (bio Instagram, story, SMS...). */
export function buildProductDirectUrl(product: { id: string; slug?: string | null }): string {
  const SITE_URL = import.meta.env.VITE_APP_URL ?? window.location.origin;
  return `${SITE_URL}/produit/${product.slug ?? product.id}`;
}
...
export function buildProductShareChannels(params: {
  ...
  const directUrl = buildProductDirectUrl({ id: params.productId, slug: params.productSlug });
  ...
  const channels: ShareChannel[] = [
    ...
    // "Copier le lien" et le partage natif utilisent le lien direct
    // /produit/:slug (lisible, sans passer par l'edge function
    // product-meta) - c'est le lien que la personne va coller telle
    // quelle quelque part (bio, story, SMS), pas un aperçu de réseau
    // social qui a besoin de l'image filigranée.
    { id: 'copy_link', label: 'Copier le lien', url: directUrl },
    { id: 'native', label: 'Plus...', url: directUrl }, // déclenche navigator.share() sur mobile si disponible
  ];
