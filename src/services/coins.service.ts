...
/** 1 pièce ≈ 12 FCFA (tarif indicatif, cf. platform_settings.coin_purchase_rate_fcfa). Utilisée pour l'affichage d'équivalents FCFA (ex. CoinDebitSummary) - l'achat de pièces lui-même utilise désormais les prix fixes de coin_purchase_tiers, pas ce taux. */
export const COIN_TO_FCFA = 12;

export interface CoinPurchaseTier {
  id: string;
  coin_amount: number;
  price_fcfa: number;
  sort_order: number;
...
/** Les 8 tranches à prix fixe (une par produit Chariow) - remplace l'ancien calcul "prix libre" (coinAmount * taux). */
export async function listCoinPurchaseTiers(): Promise<CoinPurchaseTier[]> {
  const { data, error } = await supabase
    .from('coin_purchase_tiers')
    .select('id, coin_amount, price_fcfa, sort_order')
    .eq('is_active', true)
...
/** Initie l'achat réel d'une tranche de pièces (argent réel entrant via Chariow - le seul flux financier réel que MIA gère encore). coinAmount doit correspondre exactement à l'une des 8 tranches actives. */
export async function initiateCoinPurchase(coinAmount: number, returnUrl: string) {
  const { data, error } = await supabase.functions.invoke<{ checkoutUrl: string }>('coins-purchase', {
    body: { coinAmount, returnUrl },
  });
  if (error) throw error;
...
// Tâches accordées automatiquement côté serveur (jamais par clic direct
// de l'utilisateur) - claim_reward_task les refuse explicitement. Un
// compte a exploité invite_friend_validated pour 420 pièces en cliquant
// "récupérer" 21 fois sans avoir réellement parrainé personne, avant
// que ce filtre + le blocage côté serveur ne soient ajoutés.
const AUTO_GRANTED_TASK_IDS = ['invite_friend_validated'];
...
export async function listRewardTasks(): Promise<RewardTask[]> {
  ...
  if (error) throw error;
  return ((data ?? []) as RewardTask[]).filter((task) => !AUTO_GRANTED_TASK_IDS.includes(task.id));
}
