/**
 * Chariow Service (client-side): the secret API key never touches the
 * client, only the public checkout URL and a server-verified sale lookup
 * (via the chariow-webhook Edge Function's underlying
 * settle_order_payment RPC, not a direct client call).
 */
