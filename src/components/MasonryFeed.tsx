...
export default function MasonryFeed({ country, category, personalizeCardEvery = 24 }: MasonryFeedProps) {
  ...
  const loadPage = useCallback(
    async (pageToLoad: number, replace: boolean) => {
      setLoading(true);
      try {
        const { items: newItems, hasMore: more } = await getDiscoveryFeedPage(country, pageToLoad, 12, category);
  ...
  // Recharge depuis zéro quand le pays ou la catégorie change (nouvelle
  // session de découverte), plutôt que d'empiler sur un flux obsolète.
  useEffect(() => {
    setPage(0);
    loadPage(0, true);
  }, [country, category, loadPage]);
