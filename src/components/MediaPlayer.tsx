...
export function parseVideoUrl(url: string): ParsedVideo {
  ...
  // Lien complet avec ID vidéo numérique (ex: tiktok.com/@user/video/123...)
  // -> on peut construire une iframe d'embed directe.
  const tiktokFullMatch = url.match(/tiktok\.com\/@[^/]+\/video\/(\d+)/);
  if (tiktokFullMatch) {
    return { platform: 'tiktok', embedUrl: `https://www.tiktok.com/embed/v2/${tiktokFullMatch[1]}` };
  }

  // Liens de partage courts (vm.tiktok.com/..., vt.tiktok.com/...) ou tout
  // autre lien tiktok.com (avec ou sans www, sans ID numérique extractible
  // côté client) : on reconnaît quand même la plateforme. MediaPlayer
  // retombera sur un simple lien "Voir la vidéo" tant qu'aucun embedUrl
  // n'est disponible, plutôt que de rejeter le lien.
  if (/(?:^|\/\/)(?:vm|vt)\.tiktok\.com\/[A-Za-z0-9]+/.test(url) || /(?:^|\/\/)(?:www\.)?tiktok\.com\//.test(url)) {
